...
{
	$i=static::$i;
	$sql="INSERT INTO $i$table$i ($i".implode("$i,$i",array_keys($data))."$i)VALUES(".rtrim(str_repeat('?,',count($data)),',').')';
	if($this->type=='pgsql')
	{ 
		$s=$this->query($sql.'RETURNING *',array_values($data));return$s?$s->fetchColumn(0):0;
	}
	return$this->query($sql,array_values($data))?$this->pdo->lastInsertId():0;
	
	//$s=$this->query($sql,array_values($data));
	//return$s?($this->type=='pgsql'?$s->fetchColumn(0):$this->pdo->lastInsertId()):0;
}
...
{
	$i=static::$i;
	$q="UPDATE $i$table$i SET $i".implode("$i = ?,$i",array_keys($data))."$i = ? WHERE ";list($a,$b)=self::where($where);return(($stmt=$this->query($q.$a,array_merge(array_values($data),$b)))?$stmt->rowCount():NULL);
}
